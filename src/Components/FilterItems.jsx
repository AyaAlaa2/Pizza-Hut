                 ...
                 <Row className="menuItems justify-content-center mt-5 mb-5 w-75">
                    {
                        menuItemsElement.filter((itemFil) => (
                         ...
                         <Item key={item.id} id={item.id} name={item.name} description={item.description} price={item.price} image={`/Pizza-Hut/Img/${item.image}`} loved = {item.isLoved} inCart={item.inCart} quantity={item.quantity}/> ) 
                    }
                </Row>
        )
}
